...
public abstract class MinMaxAI extends Controller {
	...
	protected @Override Location nextMove(Game g) {
		//int scoreMain = this.estimate(g.getBoard());
		
		//ArrayList<Integer> estimates = new ArrayList<>();
		Iterable<Location> locs = this.moves(g.getBoard());
		//int locSize = 1;
		//Location[] scores = new Location[locSize];
		...
		for (Location l : locs) {
			int es;
			if (pDepth > 1)
				es = this.scoreBoard(p.opponent(), g.getBoard().update(p, l), pDepth-1);
			else
				es = this.estimate(g.getBoard().update(p, l));
			mainMap.put(es, l);
		}
	...
	protected int scoreBoard(Player pTemp, Board b, int dep) {
		...
		for (Location l : locs) {
			int es = scoreBoard(pTemp.opponent(), b.update(pTemp, l), dep-1);
